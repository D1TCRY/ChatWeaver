...
Ecco come funziona (e come usarlo) l’output strutturato nel tuo sistema “Schema + Bot”.

# Cos’è `Schema` e cosa produce
La dataclass `Schema` è un wrapper comodo per costruire il `response_format` del client chat:

- `name`: nome simbolico dello schema.
- `properties`: mappa nome-campo → definizione JSON Schema (tipo, descrizione, ecc.).
- `required`: coincide con **tutte** le chiavi di `properties` (quindi ogni campo diventa obbligatorio).
- `resolve()`: restituisce il payload {"type": "json_schema", "json_schema": {...}} con:
  - `strict: True`
  - `schema.type: "object"`
  - `schema.properties: ...`
  - `schema.required: [tutte le chiavi]`
  - `additionalProperties: False`

Questo payload è passato a `response_format=...` nella chiamata `chat.completions.create(...)` e forza il modello a rispondere **solo** con un JSON che rispetta lo schema.

# Come definire uno schema
Esempio semplice (campi piatti):

```python
product_schema = Schema(
    name="product_card",
    properties={
        "title": {"type": "string", "description": "Nome del prodotto"},
        "price_eur": {"type": "number", "description": "Prezzo in euro"},
        "in_stock": {"type": "boolean"},
    }
)
```

Esempio con nested object e array (ricordando che tutti i campi in `properties` risultano obbligatori):
...
Nota: poiché `required` in `Schema` include **tutte** le chiavi di primo livello, se vuoi campi opzionali devi:
- o modificarne l’implementazione (ad es. accettando un parametro `required: list[str]`),
- o spostare il campo in un sotto-oggetto con suo `required` personalizzato.

# Come passare lo schema al Bot
Hai due modi:

1) Per richiesta singola (override):

```python
bot = Bot(...)
resp = bot.completion(
    prompt="Riassumi questo prodotto:",
    response_schema=product_schema
)
```

2) Schema di default del bot
Se imposti `self.bot_schema` nel costruttore/inizializzazione, allora puoi chiamare:

python
resp = bot.completion(prompt="Dammi una scheda prodotto")

Se non passi `response_schema` e `bot_schema` è valorizzato, verrà usato quello; altrimenti nessun vincolo di formato.

# Cosa ritorna e come leggerlo
Il metodo `completion(...)` ritorna un dict con il campo "content" che contiene **la stringa JSON** generata dal modello (perché con `response_format=json_schema` il messaggio è JSON puro). Tipicamente vorrai farne il parse:

```python
import json

data = json.loads(resp["content"])
# data è ora un dict Python conforme allo schema
...
resp = bot.completion(
    prompt="Crea una todo per 'Pagare bollo auto', priorità 3, scadenza 2025-11-01",
    response_schema=schema
)

todo = json.loads(resp["content"])
print(todo["title"], todo["priority"], todo["due_date"])
```

# Come scrivere bene `properties` (promemoria JSON Schema)
- Tipi: "string" | "number" | "integer" | "boolean" | "object" | "array".
- Per gli array usa `items: {...}` (altra definizione di schema).
- Per gli object annidati specifica le loro `properties`, `required`, `additionalProperties`.
- Puoi aggiungere vincoli utili: `enum`, `minimum/maximum`, `minLength/maxLength`, `pattern`, ecc.

# Comportamento del Bot rilevante allo schema
- Se `response_schema` è passato (o cade su `bot_schema`), il client chiama:

  ```python
  response_format = response_schema.resolve()
  ```
  quindi il modello è “costretto” a rispettare lo schema (grazie a `strict: True` e `additionalProperties: False`).

- `content` è preso da `response.choices[0].message.content`; se per qualche motivo il modello rifiuta, usa `message.refusal`.

- `history` (se presente) viene pre-inserita nei messaggi; `img_data` e `file_data` aggiungono contenuti multimodali allo stesso messaggio `user`.

# Best practice operative
- Prompting: nel prompt spiega in breve cosa vuoi e ricorda al modello che deve compilare tutti i campi dello schema. Esempio:

  Compila tutti i campi richiesti. Rispondi solo con JSON valido.

- Validazione lato client: dopo `json.loads(...)`, facoltativamente valida con una libreria JSON Schema (utile in ambienti critici).

- Campi opzionali: se ti servono, modifica `Schema.required` per accettare una lista personalizzata:
...
class Schema:
    ...
    def required(self) -> list[str]:
        return self.required_fields or [k for k in self.properties]
...
- Error handling: se il modello non riesce a soddisfare lo schema (raro con `strict: True`), potresti ricevere contenuti vuoti o refusi → gestisci eccezioni su `json.loads`.

# Mini checklist
- [ ] Definisci le `properties` (e i `required` se modifichi la classe).
- [ ] Passa lo schema a `completion(..., response_schema=...)` o configura `bot_schema`.
- [ ] Fai `json.loads` su `resp["content"]`.
- [ ] (Opzionale) Valida con JSON Schema.
- [ ] Usa i dati tipizzati nel resto dell’app.

---

## Piccole note sul codice condiviso
- Nel ramo `file_data` con lista, c’è probabilmente un refuso:
  ```
  python
  file = self.model_client.files.create(
      file=open(file_data, "rb"),  # dovrebbe essere file_path
      purpose="user_data"
  )
  ```

- Per le immagini stai fissando il mime a `image/png` nel data-URL; se carichi JPG/WebP, meglio rilevare il mime dal file.

Se vuoi, posso mostrarti una variante della classe con `required` personalizzabile e 2–3 schemi d’esempio pronti all’uso.
